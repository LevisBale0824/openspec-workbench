...
export function StreamOutput() {
  const { streamOutput, currentPhase, waitingForInput } = useWorkflowStore();

  if (currentPhase !== "executing" && !streamOutput) return null;

  const isThinking = streamOutput.includes("[思考]");
      ...
      {currentPhase === "executing" && (
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-sky-500/10 border border-sky-500/20 w-full">
          ...
          <span className="text-[11px] text-sky-400 font-medium">
            {waitingForInput ? "等待输入..." : isThinking ? "思考中..." : "正在执行..."}
          </span>
          {streamOutput && !waitingForInput && (
